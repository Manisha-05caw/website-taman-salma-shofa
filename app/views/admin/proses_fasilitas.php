...
/**
 * Validasi file upload: pastikan hanya gambar sungguhan yang diterima.
 * Cek ekstensi, MIME type dari file header (bukan browser), dan getimagesize.
...
function validateImageUpload(array $file): array
{
    ...
    if (!isset($file['tmp_name']) || !is_uploaded_file($file['tmp_name'])) {
        return ['valid' => false, 'message' => 'File tidak valid atau bukan hasil upload.'];
    }

    // 1. Cek ekstensi
    $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    if (!in_array($ext, $allowed_extensions, true)) {
        return ['valid' => false, 'message' => 'Ekstensi file tidak diizinkan. Hanya JPG, PNG, GIF, WEBP.'];
    }

    // 2. Cek MIME type dari konten file (bukan header HTTP yang bisa dipalsukan)
    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime  = finfo_file($finfo, $file['tmp_name']);
    finfo_close($finfo);
    if (!in_array($mime, $allowed_mimes, true)) {
        return ['valid' => false, 'message' => 'Tipe file tidak diizinkan. Hanya gambar yang dapat diunggah.'];
    }

    // 3. Verifikasi bahwa file benar-benar gambar (bukan payload PHP/script yang disamarkan)
...
if (isset($_POST['update'])) {
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        if (!empty($_FILES[$input_name]['tmp_name']) && is_uploaded_file($_FILES[$input_name]['tmp_name'])) {
            ...
            $validation = validateImageUpload($_FILES[$input_name]);
            if (!$validation['valid']) {
                echo "<!DOCTYPE html><html><head><script src='" . base_url('assets/js/sweetalert2.min.js') . "'></script></head><body><script>Swal.fire({title: 'Upload Ditolak!', text: " . json_encode($validation['message']) . ", icon: 'error'}).then(() => { window.history.back(); });</script></body></html>";
                exit;
            }
            // ----------------------------------
    ...
    echo "<!DOCTYPE html><html><head><script src='" . base_url('assets/js/sweetalert2.min.js') . "'></script></head><body>
    <script>
...
if (isset($_POST['simpan'])) {
    ...
    if (!empty($_FILES['gambar']['tmp_name']) && is_uploaded_file($_FILES['gambar']['tmp_name'])) {
        // ---- VALIDASI KEAMANAN GAMBAR ----
        $validation = validateImageUpload($_FILES['gambar']);
        if (!$validation['valid']) {
            echo "<!DOCTYPE html><html><head><script src='" . base_url('assets/js/sweetalert2.min.js') . "'></script></head><body><script>Swal.fire({title: 'Upload Ditolak!', text: " . json_encode($validation['message']) . ", icon: 'error'}).then(() => { window.history.back(); });</script></body></html>";
            exit;
        }
        ...
        if (compressToWebP($_FILES['gambar']['tmp_name'], $folder)) {
            ...
            // Tampilkan error jika unggah gagal
             echo "<!DOCTYPE html><html><head><script src='" . base_url('assets/js/sweetalert2.min.js') . "'></script></head><body><script>Swal.fire({title: 'Error!', text: 'Gagal memindahkan dan kompresi file. Periksa izin folder.', icon: 'error'}).then(() => { window.history.back(); });</script></body></html>";
        }
    }
