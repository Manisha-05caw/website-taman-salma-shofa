...
<head>
    <link rel="icon" type="image/webp" href="<?= base_url('assets/logo.webp') ?>">
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Fasilitas & Layanan - Taman Salma Shofa</title>
    <meta name="description" content="Jelajahi berbagai fasilitas lengkap di Taman Salma Shofa. Tersedia kolam renang, gazebo nyaman, penyewaan baju tradisional, dan area rekreasi keluarga.">
    <meta name="keywords" content="fasilitas taman salma shofa, kolam renang samarinda, sewa baju tradisional samarinda, wisata keluarga samarinda">
...
<body class="bg-[#FAFAFB] text-gray-800 flex flex-col min-h-screen overflow-x-hidden">
    ...
    <main class="max-w-[1240px] mx-auto px-6 md:px-12 pt-[100px] pb-12 w-full flex-grow">
                    ...
                    ?>
                            <div class="flex flex-col rounded-3xl overflow-hidden shadow-md group/card shrink-0 w-[300px] md:w-[360px] snap-start">
                                <div class="h-[250px] w-full overflow-hidden bg-gray-200 relative">
                                    <img src="<?= $img_final ?>" 
                                         alt="<?= htmlspecialchars($nama_f) ?>" loading="lazy"
        ...
        <section class="mb-20">
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
                <div class="border-l-[4px] border-[#E88D57] pl-5">
            ...
            <div class="relative group">
                ...
                <div id="slider-pendukung" class="flex overflow-x-auto gap-6 pb-8 snap-x snap-mandatory hide-scroll scroll-smooth relative px-2 py-4 items-stretch">
                    ...
                    ?>
                        <div class="flex flex-col h-full bg-[#E88D57] rounded-[24px] shadow-sm hover:shadow-md transition-shadow border border-[#E88D57] overflow-hidden shrink-0 w-[240px] md:w-[280px] snap-start group/card2">
                            <div class="h-[160px] w-full overflow-hidden bg-gray-200 border-b border-[#E88D57]">
                                <img src="<?= $img_p ?>" class="w-full h-full object-cover group-hover/card2:scale-105 transition-transform duration-700" alt="<?= htmlspecialchars($pendukung['nama'] ?? '') ?>" loading="lazy">
                            </div>
                            <div class="p-5 flex-grow flex flex-col text-left">
                                <h4 class="font-bold text-white text-[17px] mb-2 leading-tight"><?= htmlspecialchars($pendukung['nama'] ?? '') ?></h4>
                                <div class="text-white/90 text-[13px] leading-relaxed">
                                    <?= htmlspecialchars(batasi_kata($pendukung['deskripsi'] ?? '', 15)) ?>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
            ...
            <div class="w-full lg:w-1/2 relative z-10 flex gap-4 h-[380px]">
                <div class="flex flex-col gap-4 w-1/2 h-full">
                    <div class="relative w-full h-[182px] rounded-[20px] overflow-hidden bg-gray-200 group">
                        <img src="<?= htmlspecialchars($gambar_baju[0]) ?>" onerror="this.src='assets/homepage.jpg'" loading="lazy" class="object-cover w-full h-full group-hover:scale-110 transition-transform duration-700">
                    </div>
                    <div class="relative w-full h-[182px] rounded-[20px] overflow-hidden bg-gray-200 group">
                        <img src="<?= htmlspecialchars($gambar_baju[1]) ?>" onerror="this.src='assets/homepage.jpg'" loading="lazy" class="object-cover w-full h-full group-hover:scale-110 transition-transform duration-700">
                    </div>
                </div>
                <div class="w-1/2 h-full">
                    <div class="relative w-full h-[380px] rounded-[20px] overflow-hidden bg-gray-200 group">
                        <img src="<?= htmlspecialchars($gambar_baju[2]) ?>" onerror="this.src='assets/homepage.jpg'" loading="lazy" class="object-cover w-full h-full group-hover:scale-110 transition-transform duration-700">
                    </div>
                </div>
            </div>
        </section>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            // Slider initialization or other scripts can go here
        });
    </script>
</body>
</html>